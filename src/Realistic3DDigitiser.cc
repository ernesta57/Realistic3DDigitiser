...
Realistic3DDigitiser::Realistic3DDigitiser(const std::string& name, ISvcLocator* svcLoc) : MultiTransformer(name, svcLoc,
...
StatusCode Realistic3DDigitiser::initialize() {
    ...
    // 3D sensor column geometry: pitch defaults to pixel size (one
    // readout column per pixel)
    m_columnGrid.pitchX = (m_columnPitchX > 0) ? m_columnPitchX.value() : m_pixelSizeX.value();
    m_columnGrid.pitchY = (m_columnPitchY > 0) ? m_columnPitchY.value() : m_pixelSizeY.value();
    m_columnGrid.columnRadius = m_columnRadius.value();
...
StatusCode Realistic3DDigitiser::LoadGeometry() {
    ...
    if (m_ChargeDigitizeNumBits == 4 && isVertex && m_layerThickness[0]==0.075) m_DigitizedBins = {500, 675, 910, 1228, 1656, 2235, 3015, 4067, 5487, 7403, 9987, 13473, 18177, 24523, 33084, 44634}; //75 microns
    if (m_ChargeDigitizeNumBits == 4 && isVertex && m_layerThickness[0]==0.1) m_DigitizedBins = {500, 688, 946, 1300, 1788, 2460, 3382, 4652, 6397, 8797, 12098, 16638, 22881, 31467, 43274, 59512}; //100 microns
    if (m_ChargeDigitizeNumBits == 4 && isVertex && m_layerThickness[0]==0.2) m_DigitizedBins = {500, 720, 1037, 1494, 2152, 3099, 4463, 6428, 9258, 13334, 19205, 27660, 39838, 57378, 82640, 119024}; //200 microns
    if (m_ChargeDigitizeNumBits == 4 && isVertex && m_layerThickness[0]==0.4) m_DigitizedBins = {500, 754, 1138, 1716, 2588, 3904, 5889, 8883, 13399, 20211, 30486, 45985, 69363, 104626, 157816, 238048}; //400 microns
    ...
    if (!isVertex) {
        ...
        float scalefactor = 2.; 
        for (int i = 0; i < (int)m_DigitizedBins.size(); i++){
...
std::tuple<edm4hep::SimTrackerHitCollection,
           ...
           edm4hep::TrackerHitSimTrackerHitLinkCollection> Realistic3DDigitiser::operator()(
    ...
    for (int i=0; i < nSimHits; ++i) {
        ...
        intState.currentLayer = Realistic3DDigitiser::layerMapping(cellID_coder.get(simTrkHit.getCellID(), "layer"));
        ...
        if (m_timeSmearingSigma > 0) TimeSmearer(simTrkHitVec, &intState);
...
StatusCode Realistic3DDigitiser::finalize() {
  delete m_fluctuate;
  return StatusCode::SUCCESS;
}
 ...
 * 
 */
void Realistic3DDigitiser::FindLocalPosition(edm4hep::SimTrackerHit &hit, 
...
void Realistic3DDigitiser::ProduceIonisationPoints(edm4hep::SimTrackerHit &hit, InternalState *intState) const{
    ...
    if (m_doMultipleScattering) {
        ...
        double msPos[3] = {loopPos[0], loopPos[1], loopPos[2]};
        ...
        // find final exit point from the scattered path
        for (int i = 0; i < 2; ++i) {
            exit[i] = msPos[i] + msDir[i] * (exit[2] - msPos[2]) / msDir[2];
        }
        // propagate the net scattered direction downstream (tanx/tany below
        // read from `dir`); position/currentLocalPosition intentionally
        // stays unscattered.
        dir.x = msDir[0];
        dir.y = msDir[1];
        dir.z = msDir[2];
...
void Realistic3DDigitiser::ProduceSignalPoints(InternalState *intState) const{
    ...
    for (int i = 0; i < intState->numberOfSegments; ++i) {
        ...
        debug() << "- " << i << ": charge=" << charge
            << ", x="<< colX << "(delta=" << colX - x << ")"
            << ", y="<< colY << "(delta=" << colY - y << ")"
            << ", distanceToColumn=" << DistanceToColumn
            << ", sigma=" << SigmaDiff << endmsg;
    }
}

void Realistic3DDigitiser::ProduceHits(MutableSimTrackerHitVec &simTrkVec, edm4hep::SimTrackerHit &simHit, InternalState *intState) const{  
    simTrkVec.clear();
 ...
 * distribution...
 */
void Realistic3DDigitiser::PoissonSmearer(MutableSimTrackerHitVec &simTrkVec) const{
 ...
 */
void Realistic3DDigitiser::GainSmearer(MutableSimTrackerHitVec &simTrkVec) const{
 ...
 */
void Realistic3DDigitiser::ApplyThreshold(MutableSimTrackerHitVec &simTrkVec) const{
 ...
 */
void Realistic3DDigitiser::ChargeDigitizer(MutableSimTrackerHitVec &simTrkVec) const{
...
void Realistic3DDigitiser::TimeSmearer(MutableSimTrackerHitVec &simTrkVec, InternalState *intState) const{
    ...
    float sigma_landau = (m_sigma_landauOverride > -1.0) ? m_sigma_landauOverride.value() : sigma_landauDefault;
    float sigma_timewalk = (m_sigma_timewalkOverride > -1.0) ? m_sigma_timewalkOverride.value() : sigma_timewalkDefault;
    float sigma_jitter = (m_sigma_jitterOverride > -1.0) ? m_sigma_jitterOverride.value() : sigma_jitterDefault;
    float sigma_TDC = (m_sigma_TDCOverride > -1.0) ? m_sigma_TDCOverride.value() : sigma_TDCDefault;
    float sigma_clock = (m_sigma_clockOverride > -1.0) ? m_sigma_clockOverride.value() : sigma_clockDefault;

    float sigma_total = std::sqrt(sigma_landau * sigma_landau + sigma_timewalk * sigma_timewalk
 ...
 * Discretization based on number of bits and bin width scheme.
 */
void Realistic3DDigitiser::TimeDigitizer(MutableSimTrackerHitVec &simTrkVec) const{
 ...
 */
void Realistic3DDigitiser::ReconstructTrackerHit(MutableSimTrackerHitVec &simTrkVec, 
                                              TempRecoHit *info, 
                                              InternalState *intState) const{
    edm4hep::Vector3d pos(0, 0, 0);
    ...
    debug() << "Edge sizes, minx, maxx, miny, maxy: " << edge_size_minx << ", "
            ...
            << " (no Lorentz-angle correction applied -- not used by the"
            << " 3D lateral-drift transport model, see ProduceSignalPoints)";
 ...
 */
void Realistic3DDigitiser::TransformToLab(const int cellID, const edm4hep::Vector3d &xLoc, edm4hep::Vector3d &xLab) const{
 ...
 */
void Realistic3DDigitiser::TransformXYToCellID(double x, double y, int & ix, int & iy, InternalState *intState) const{
...
*/
void Realistic3DDigitiser::TransformCellIDToXY(int ix, int iy, double & x, double & y, InternalState *intState) const{
    int layer = intState->currentLayer;
...
int Realistic3DDigitiser::GetPixelsInaColumn(InternalState *intState) const{//SP: why columns!?! I would have guess row..
    if (isBarrel){
...
int Realistic3DDigitiser::GetPixelsInaRow(InternalState *intState) const{
    if (isBarrel){
...
void Realistic3DDigitiser::PrintGeometryInfo() {
...
//=============================================================================
double Realistic3DDigitiser::randomTail( const double qmin, const double qmax ) const{
...
int Realistic3DDigitiser::layerMapping( int id) const {
  int mappedLayerID = -1;
